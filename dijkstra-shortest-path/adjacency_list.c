...
void graph_insert(GRAPH* graph, int convergent, int divergent, int weight) {
    if(graph != NULL) {
        list_insert(graph->adjacency_list[convergent], divergent, weight); 
        list_insert(graph->adjacency_list[divergent], convergent, weight); 
    }
    return;
}
